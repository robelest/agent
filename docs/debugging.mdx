...
AI SDK 7 omits request and response bodies by default. Agent requests the
available bodies when `rawRequestResponseHandler` is configured. Text results
include `response.messages`; object results may omit it.

Raw provider bodies can contain Convex-reserved field names such as `$schema`.
Before inserting a body as a Convex object, encode it as a string or recursively
sanitize reserved `$`-prefixed keys.
...
AI SDK 7 emits OpenTelemetry spans after you register the `@ai-sdk/otel`
integration. Install it in your application:
...
Registration enables telemetry for all AI SDK calls. Use the stable `telemetry`
option to attach a function ID or to opt out for one call. Flush spans before
the handler returns so their exports are
