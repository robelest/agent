...
You can consume the stream in all the ways supported by the AI SDK, including
iterating over its content or passing `result.stream` to a stateless response
helper.
...
async function stream(ctx: ActionCtx, threadId: string, order: number) {
  ...
  return {
    // Pass response.stream to toTextStream/createTextStreamResponse for HTTP.
    response,
