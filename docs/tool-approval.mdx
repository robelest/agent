...
AI SDK 7 keeps approval policy at the generation call site. Define tools
normally, then pass `toolApproval` to `generateText` or `streamText`. A policy
entry can always require approval or decide from the proposed input:
...
Tools omitted from `toolApproval`, and policy functions that return `undefined`,
execute immediately. `createTool({ needsApproval })` remains as a deprecated
compatibility path for AI SDK 6 applications.
